...
class Grid:
    ...
    def __init__(self):
        ...
        self.ang_params = (0.15, 0.8, 3.8, 0.4)
        self.ori_params = (0.18, 0.8, 3.8, 0.4)
        self.eb = False
    ...
    def _discretize_phi(self, r):
        ...
        if n == 1: 
            ...
            if self.eb: return np.linspace(mini-da, maxi, n+1)
            return np.linspace(mini, maxi, n)
    ...
    def _discretize_theta(self, r, phi):
        ...
        if n == 1: 
            ...
            da = l / (n-1)
            if self.eb: return np.linspace(mini-da, maxi+da, n+2)
            return np.linspace(mini, maxi, n)
    ...
    def _find_neighbors4_0(self, xs, my_x):
        n = len(xs)
        if n <= 4:
            return range(len(xs))
        i = 1
    ...
    def _interp_1D0(self, xs, ys, my_x):
        if len(xs) == 1:
            return ys[0]
        if len(xs) == 2:
            x0, x1 = xs
        ...
        if len(xs) == 4:
            x0, x1, x2, x3 = xs
            y0, y1, y2, y3 = ys
            d1 = ((y1 - y0) / (x1 - x0) + (y2 - y1) / (x2 - x1)) / 2
            d2 = ((y2 - y1) / (x2 - x1) + (y3 - y2) / (x3 - x2)) / 2
            A = np.array([[1, x1, x1**2, x1**3], [1, x2, x2**2, x2**3], [0, 1, 2*x1, 3*x1**2], [0, 1, 2*x2, 3*x2**2]])
            y = np.array([y1, y2, d1, d2])
            a0, a1, a2, a3 = np.linalg.solve(A, y)
            return a0 + a1*my_x + a2*my_x**2 + a3*my_x**3
